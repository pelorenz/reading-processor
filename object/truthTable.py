...
class TruthTable:
    ...
    def initialize(s, range, refMSS, qcaSet, model):
        c = s.config

        s.range_id = range
        s.refMS_IDs = refMSS
        s.qcaSet = qcaSet
        s.variantModel = model

    def generate(s):
        c = s.config

        s.info('Running TTMaker on range', s.range_id)

        s.qcaMSS = c.get('qcaSets')[s.qcaSet]['qcaMSS']
        s.booleanAggregates = c.get('qcaSets')[s.qcaSet]['aggregates']
